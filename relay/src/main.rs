...
mod udp;
use udp::parser::{RelayEvent, RelayResponse};

use relay::RelayManager;

use crate::udp::parser::udp_handler;
struct Appstate {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let udp_listener =
        std::net::UdpSocket::bind("0.0.0.0:9903").expect("Failed to bind udp socket");
    ...
    println!("UDP Socket Listening on {:?}", udp_listener.local_addr());
    udp_handler(udp_listener, app_handle).await;
    Ok(())
}
